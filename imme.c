...
void imme_draw_gfx(uint8_t  dstX,     // ypos in pixel of target area
                   uint8_t  dstY,     // ypos in pixel of target area
                   uint8_t  dstWidth, // width in pixel of target area
                   uint8_t  dstHeight,// height in pixel of target area
				   uint8_t *srcData,  // pointer to src data
				   uint8_t  srcX,     // src x pos to use
				   uint8_t  srcY,     // src y pos to use
				   uint8_t  srcWidth) // width of srcData 
{
	uint8_t *dst;
//	uint8_t *prevDst;
	uint8_t *src;
	uint8_t *prevSrc;
	uint8_t *ps;
	uint8_t srcYOff, srcYOffInv;
	uint8_t hDiv, hRem;
	uint8_t x,p,dStep,sStep;

	EA = 0;

	dst        = dispBuf + (dstY >> 3) * (DISP_WIDTH*2L) + dstX;
	src        = srcData + (srcY >> 3) * (srcWidth*2L)   + srcX;
	srcYOff    = dstY & 7;
	srcYOffInv = 8 - srcYOff;
	hDiv       = dstHeight >> 3;
	hRem       = dstHeight &  7;
	dStep      = DISP_WIDTH - dstWidth;
	sStep      = srcWidth   - dstWidth;
	// copy first line
	prevSrc = src;
//	prevDst = dst;
	// LSB of color
